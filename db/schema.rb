...
ActiveRecord::Schema[7.0].define(version: 20_221_211_201_511) do # rubocop:disable Metrics/BlockLength
  # These are extensions that must be enabled in order to support this database
  enable_extension 'plpgsql'

  # Custom types defined in this database.
  # Note that some types may not work with other database engines. Be careful if changing database.
  create_enum 'friendship_status', %w[inactive active unfriended rejected]
  ...
  create_table 'note_labels', force: :cascade do |t|
    ...
    t.index %w[note_id label_id], name: 'index_note_labels_on_note_id_and_label_id', unique: true
  ...
  create_table 'notes', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index "to_tsvector('english_nostop'::regconfig, (((title)::text || ' '::text) || content))",
            name: 'fts_index_notes_on_title_and_content', using: :gin
